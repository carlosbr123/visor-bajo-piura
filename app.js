...
let CORR_S = null;   // lo mismo para SONICS (`datos/sonics_correccion.json`, de sonics_sesgo.py); su pronóstico lo baja la tarea diaria
...
const TX = {
  es: {
    ...
    pro_pron: (fu, dias, q, f, a, b, niv) => `<b>Pronóstico ${fu}</b>, próximos ${dias} días: caudal más alto previsto <b>${q} m³/s</b> el ${f} (${niv}) <span class="rango">· los escenarios, entre ${a} y ${b}</span>`,
    ...
    pro_desempeno: (fu, pod, n, far, ret, cumple) => `<b>${cumple ? 'Desempeño medido' : 'Cuidado'}:</b> probado en 2021–2026 con la misma corrección, ${fu} anticipó el ${pod} % de los ${n} días con más de 900 m³/s, y el ${far} % de sus avisos fueron falsos${Math.abs(ret) >= 5 ? `; en 2023 su pico llegó ${Math.abs(ret)} días ${ret > 0 ? 'tarde' : 'antes'}` : ''}.${cumple ? '' : ' Úselo como tendencia, no como alerta.'}`,
    ...
    pro_fuente_s: (dir, f) => `SENAMHI, Hidrovisor SONICS, tramo del río en Piura, corregido con el caudal observado de 2014–2020 · pronóstico del ${f}. Resultados referenciales del SENAMHI.`,
    pro_tope: q => ` La corrección no pasa de ${q} m³/s, el mayor caudal observado.`,
  ...
  en: {
    ...
    pro_pron: (fu, dias, q, f, a, b, niv) => `<b>${fu} forecast</b>, next ${dias} days: highest expected discharge <b>${q} m³/s</b> on ${f} (${niv}) <span class="rango">· scenarios between ${a} and ${b}</span>`,
    pro_bajo_am: 'below the yellow alert', pro_ver: 'show on map',
    pro_extremo: (q, f) => `Highest of all scenarios: ${q} m³/s on ${f}.`,
    pro_desempeno: (fu, pod, n, far, ret, cumple) => `<b>${cumple ? 'Measured performance' : 'Caution'}:</b> tested over 2021–2026 with the same correction, ${fu} anticipated ${pod}% of the ${n} days above 900 m³/s, and ${far}% of its warnings were false${Math.abs(ret) >= 5 ? `; in 2023 its peak came ${Math.abs(ret)} days ${ret > 0 ? 'late' : 'early'}` : ''}.${cumple ? '' : ' Use it as a trend, not as an alert.'}`,
    ...
    pro_fuente_s: (dir, f) => `SENAMHI, SONICS viewer, river reach at Piura, corrected with the observed discharge of 2014–2020 · forecast of ${f}. SENAMHI states its results are indicative.`,
    pro_tope: q => ` The correction does not go above ${q} m³/s, the highest observed discharge.`,
...
async function cargar() {
  ...
  CORR_S = await fetch('datos/sonics_correccion.json').then(r => r.ok ? r.json() : null).catch(() => null);
...
function mostrarCaudalExterno() {
  ...
  if (FQ === 'obs') {
    ...
    const gu = ((EXT && EXT.pronosticos) || []).find(p => p.clave === FQ && p.serie && p.serie.length);
    const p = (FQ === 'glofas' && PRON.glofas) || (gu && Object.assign({}, gu, { directo: false, serie: gu.serie.filter(e => e.f >= hoyLima()) }));
    const co = FQ === 'glofas' ? CORR : CORR_S, nombre = FQ === 'glofas' ? 'GloFAS' : 'SONICS';
    if (!p || !p.serie.length) {
      h += `<div>${tr('pro_sin', nombre)}</div>`;
      if (FQ === 'sonics') h += `<div class="rango">${tr('pro_sonics')} <a href="https://www.senamhi.gob.pe/sonics/" target="_blank" rel="noopener">${tr('pro_sonics_ir')}</a></div>`;
    } else {
      let mx = p.serie[0], ex = p.serie[0];
      for (const e of p.serie) { if (e.med > mx.med) mx = e; if ((e.max == null ? -1 : e.max) > (ex.max == null ? -1 : ex.max)) ex = e; }
      h += `<div>${tr('pro_pron', nombre, p.serie.length, fmtQ(mx.med), fmtFecha(mx.f), fmtQ(mx.p25 != null ? mx.p25 : mx.min), fmtQ(mx.p75 != null ? mx.p75 : mx.max), nivelDe(mx.med))}<button data-qext="${mx.med}">${tr('pro_ver')}</button></div>`;
      h += graficoPron(p.serie);
      if (ex.max != null) h += `<div class="rango">${tr('pro_extremo', fmtQ(ex.max), fmtFecha(ex.f))}</div>`;
      const pb = co && co.prueba_2021_2026;
      if (pb) h += `<div class="desempeno">${tr('pro_desempeno', nombre, Math.round(pb.POD_900 * 100), pb.dias_obs_900, Math.round(pb.FAR_900 * 100), pb.retraso_pico_2023_dias, co.criterio_cumplido === true)}</div>`;
      const tope = co && co.qo && co.qo[co.qo.length - 1];
      h += `<div class="rango" id="pro-nota" data-modo="${p.directo ? 'directo' : 'guardado'}">${tr(FQ === 'glofas' ? 'pro_fuente' : 'pro_fuente_s', p.directo, fmtFecha(p.consultado))}` +
        `${mx.med >= 0.7 * tope ? tr('pro_tope', fmtQ(tope)) : ''} ${tr('ext_nota', '')}</div>`;
    }
  }
